...
def generateENETRandom(image_limit, folder_limit, main_dir, frame_pre, frame_ext):
    while True:
        ...
        i = getRandom(20, arr[folder] - 1) #starting from 2 since 2nd video had corrupted first frame (LUL) and I'm too lazy to rename all the images
        x1 = getImage(main_dir, source, '', str(i), frame_ext)
        x2 = getImage(main_dir, source, '', str(i+1), frame_ext)
        g  = getImage(main_dir, source, '', str(i+2), frame_ext)
...
def generateYNETRandom(image_limit, folder_limit, main_dir, frame_pre, audio_pre, frame_ext, audio_ext):
    while True:
        #folder = getRandom(1, folder_limit)
        folder = 6
        source = str(folder)
